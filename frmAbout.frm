...
Begin VB.Form frmAbout 
   ...
   Begin VB.PictureBox picLeft 
      ...
      Begin VB.Label Label9 
         ...
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "ProcessXP© Salex Software© 2003"
         BeginProperty Font 
            Name            =   "Verdana"
            Size            =   6
            Charset         =   0
...
Private Sub Form_Activate()
 TransPrep Me.hwnd 'Prepare window object for Transparency; See TransPrep function..
  If sndSupported = True Then sndClass.doAbout 'Use Sound Server to play 'About' sound
End Sub

Private Sub Form_Load()
On Error Resume Next
Dim rUser$, rCompany$ 'Dimensionalize variables rUser as string data type, rCompany as string data type
 VGrad picLeft.hdc, picLeft.ScaleHeight, picLeft.ScaleWidth, COLOR_BTNFACE, white2black
 'Use ProcessXP GUI DLL to draw a vertical gradient using System Color ButtonFace, see ProcessXP GUI DLL C++ Project for more info...
  VGrad picBot.hdc, picBot.ScaleHeight, picBot.ScaleWidth, COLOR_BTNFACE, Black2White
  'Use ProcessXP GUI DLL to draw a vertical gradient using System Color ButtonFace, see ProcessXP GUI DLL C++ Project for more info...
   lblVersion.Caption = App.Major & "." & App.Minor & "." & App.Revision
   'Update Version label with the global version variables: Major, Minor and Revision
    If Mid$(App.Path, Len(App.Path) - 1) = "\" Then
     lblCompiled.Caption = "Compiled on " & FileDateTime(App.Path & App.EXEName & ".exe")
     'Update 'Compiled on' label with the FileDateTime(Modified) of this file, this will not return a value if you are debugging...
    Else
     lblCompiled.Caption = "Compiled on " & FileDateTime(App.Path & "\" & App.EXEName & ".exe")
     'Update 'Compiled on' label with the FileDateTime(Modified) of this file, this will not return a value if you are debugging...
    End If
     rUser = GetString(HKEY_LOCAL_MACHINE, "Software\Salex Software\ProcessXP\RegUser", "UserName")
      ...
      'Note that these registry keys are set by the InstallShield installation script which install this application
       If rUser <> "" Then lblName.Caption = rUser Else lblName.Caption = GetString(HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows NT\CurrentVersion", "RegisteredOwner")
        ...
        'If these registry keys are not set, than use the Operating Systems registrant information...
         If lblName.Caption = "" Then lblName.Caption = "Can't Retreive Name": lblName.ForeColor = vbRed
          ...
          'If the Operating systems registrant information reigistry keys aren't set either, than update registrant labels...
End Sub
...
Private Sub Label6_Click()
 ...
 'Use Shell's ShellExecute function to determine the Executable which handles this type of document (mailto:)
 'The lpOperation argument specifies the Verb context(open, edit, print, explore) under which to execute the specified docuement
 'Alternative function: FindExecutable
End Sub
